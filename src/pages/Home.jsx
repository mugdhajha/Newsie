...
const Home = () => {
  const [cat, setCat] = useState("Business"); // yeah, category
  const [items, setItems] = useState([]);     // holds articles
  const [spin, setSpin] = useState(false);    // for loading
  ...
  useEffect(() => {
    fetchNews(); // refresh when cat changes
  }, [cat]);
